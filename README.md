...
This code builds on methods described in Gold and Cowgill, 2011 (EPSL), and is based on work developed in Zinke et al., 2017 (GRL) and Zinke et al., 2019 (GRL). These codes use Python 3.x, and were tested on versions 3.6 - 3.9.
...
* analyticalSlipRates.py - Similar to ```calcSlipRates.py```, this function will compute the incremental slip rates and output the results as PDFs. In this case, however, the calculations are performed using analytical formulations rather than bootstrap sampling. NOTE: This function does not yield valid results when measurements overlap within uncertainty! An error will be thrown if uncertainties in age or displacement overlap. Function syntax is similar to that of ```calcSlipRates.py```, e.g., ```analyticalSlipRates.py DspAgeData.yaml --pdf-analysis HPD```
